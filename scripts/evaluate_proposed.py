"""
Proposed Method Multi-Document Summarization Evaluation
Evaluates importance-aware multi-document summaries using ROUGE and BERTScore
Uses identical evaluation methodology to baseline for fair comparison
"""
...
# Load proposed summaries
print("Loading proposed_summaries.csv...")
df = pd.read_csv('data/processed/proposed_summaries.csv')

initial_count = len(df)
print(f"  Loaded {initial_count} rows")

# Remove rows with missing or empty summaries
df = df.dropna(subset=['proposed_generated_summary', 'reference_summary'])
df = df[df['proposed_generated_summary'].str.strip() != '']
...
std_rouge1 = np.std(rouge_scores['ROUGE-1'])
avg_rouge2 = np.mean(rouge_scores['ROUGE-2'])
std_rouge2 = np.std(rouge_scores['ROUGE-2'])
avg_rougel = np.mean(rouge_scores['ROUGE-L'])
std_rougel = np.std(rouge_scores['ROUGE-L'])

print(f"[OK] ROUGE scores computed")
print(f"  Average ROUGE-1: {avg_rouge1:.4f} +/- {std_rouge1:.4f}")
print(f"  Average ROUGE-2: {avg_rouge2:.4f} +/- {std_rouge2:.4f}")
print(f"  Average ROUGE-L: {avg_rougel:.4f} +/- {std_rougel:.4f}")
...
generated_summaries = df['proposed_generated_summary'].tolist()
...
std_bertscore_f1 = np.std(F1.cpu().numpy())
avg_bertscore_p = np.mean(P.cpu().numpy())
std_bertscore_p = np.std(P.cpu().numpy())
avg_bertscore_r = np.mean(R.cpu().numpy())
std_bertscore_r = np.std(R.cpu().numpy())

print(f"[OK] BERTScore computed")
print(f"  Average BERTScore Precision: {avg_bertscore_p:.4f} +/- {std_bertscore_p:.4f}")
print(f"  Average BERTScore Recall: {avg_bertscore_r:.4f} +/- {std_bertscore_r:.4f}")
print(f"  Average BERTScore F1: {avg_bertscore_f1:.4f} +/- {std_bertscore_f1:.4f}")
...
output_file = 'data/processed/proposed_evaluation_results.csv'
...
rouge_stds = [std_rouge1, std_rouge2, std_rougel]
...
ax.set_title('Proposed Method: Overall ROUGE Scores\n(Importance-Aware Multi-Document Summarization)', 
...
plt.tight_layout()
fig_file1 = 'data/processed/proposed_rouge_scores.png'
...
ax.set_title('Proposed Method: ROUGE-L Performance by News Category\n(Importance-Aware Multi-Document Summarization)', 
             fontsize=14, fontweight='bold', pad=20)
ax.set_xlim([0, 1.0])
ax.grid(True, alpha=0.3, axis='x')

plt.tight_layout()
fig_file2 = 'data/processed/proposed_categorywise_rouge.png'
...
ax.set_ylabel('News Category', fontsize=12, fontweight='bold')
ax.set_title('Proposed Method: BERTScore F1 Distribution by News Category\n(Importance-Aware Multi-Document Summarization)', 
             fontsize=14, fontweight='bold', pad=20)
ax.set_xlim([0, 1.0])
ax.grid(True, alpha=0.3, axis='x')

plt.tight_layout()
fig_file3 = 'data/processed/proposed_bertscore_distribution.png'
...
print(f"  BERTScore Precision: {avg_bertscore_p:.4f} ± {std_bertscore_p:.4f}")
print(f"  BERTScore Recall: {avg_bertscore_r:.4f} ± {std_bertscore_r:.4f}")
print(f"  BERTScore F1: {avg_bertscore_f1:.4f} ± {std_bertscore_f1:.4f}")
...
print(f"  - Proposed method: Importance-Aware Multi-Document Summarization")
print(f"  - Evaluation metrics: ROUGE-1/2/L, BERTScore F1")
print(f"  - Results are ready for comparison with baseline")
print(f"  - All visualizations are publication-quality (300 DPI)")

print("\nNext steps:")
print(f"  - Review proposed_evaluation_results.csv for detailed per-cluster scores")
print(f"  - Compare with baseline_evaluation_results.csv")
print(f"  - Generate comparison visualizations and statistical analysis")
